...
export default function TeacherGradingDashboard() {
  ...
  // ✅ NEW: States for the exam dropdown
  ...
  // 1. Fetch all exams to populate the dropdown
  useEffect(() => {
    api.get('/exams')
  ...
  // 2. Fetch pending grades whenever the selected exam changes
  useEffect(() => {
    console.log('🔍 Checking selectedExamId:', selectedExamId);
    if (selectedExamId) {
  ...
  const handleApprove = async (logId: string, useAiScore: boolean) => {
    try {
      ...
      fetchPendingGrades(selectedExamId); // Refresh list
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* ✅ NEW: Exam Selector Dropdown */}
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col sm:flex-row items-center gap-4">
        <label className="font-semibold text-gray-700 whitespace-nowrap">Select Exam to Grade:</label>
        <select 
          value={selectedExamId} 
          onChange={(e) => setSelectedExamId(e.target.value)}
          className="flex-1 w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-12 text-center border border-gray-100">
          ...
          <p className="text-gray-500 mt-2">
            {selectedExamId ? 'There are no pending theory answers to grade for this exam.' : 'Please select an exam from the dropdown above.'}
          </p>
        </div>
      ) : (
