...
export default function AddSubjectModal({ isOpen, onClose, onSubjectSaved, editSubject }: AddSubjectModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editSubject) {
        await api.put(`/subject/${editSubject.id}`, formData);
      } else {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button type="submit" disabled={loading} className="px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 transition disabled:opacity-50">
              {loading ? 'Saving...' : (editSubject ? 'Update Subject' : 'Save Subject')}
            </button>
          </div>
        </form>
      </div>
