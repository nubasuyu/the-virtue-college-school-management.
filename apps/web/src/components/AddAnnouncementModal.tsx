...
export default function AddAnnouncementModal({ isOpen, onClose, onSaved, editItem }: AddAnnouncementModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        await api.patch(`/announcement/${editItem.id}`, formData);
      } else {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea required name="content" value={formData.content} onChange={handleChange} rows={5}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button type="submit" disabled={loading} className="px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 transition disabled:opacity-50">
              {loading ? 'Publishing...' : (editItem ? 'Update' : 'Publish Announcement')}
            </button>
          </div>
        </form>
      </div>
